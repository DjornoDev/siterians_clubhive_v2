            ...
            <div class="p-6 overflow-y-auto flex-grow">
                <form id="edit-club-post-form"
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="post_caption" id="post_caption" x-model="editPostCaption"
                    ...
                    <div class="mb-4">
                        ...
                        <div>
                            <label for="file_attachments_edit" class="block text-sm font-medium text-gray-600 mb-2">
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-md p-4 text-center hover:border-blue-400 transition-colors"
                                id="editDropZone" ondrop="dropEditHandler(event);"
                                ondragover="dragOverEditHandler(event);" ondragleave="dragLeaveEditHandler(event);">
                                <input type="file" name="file_attachments[]" id="file_attachments_edit"
                                    class="hidden" accept=".pdf,.doc,.docx,.txt,.ppt,.pptx,.xls,.xlsx,.zip,.rar"
                                    multiple onchange="handleEditFileSelect(event)">
                                <div class="text-gray-600">
                                    ...
                                    <p class="text-sm">
                                        <button type="button" class="text-blue-600 hover:text-blue-500 font-medium"
                                            onclick="document.getElementById('file_attachments_edit').click()">
...
<script>
    // Multiple file upload handling for post edit
    ...
    function processEditFiles(files) {
        ...
        if (selectedEditFiles.length + files.length > 5) {
            errors.push(
                `Cannot upload more than 5 files. Currently selected: ${selectedEditFiles.length}, trying to add: ${files.length}`
            );
        }

        // Validate each file
        files.forEach(file => {
            const validation = validateEditFile(file);
            if (!validation.valid) {
    ...
    function updateEditFileList() {
        ...
        selectedEditFiles.forEach((file, index) => {
            ...
            <button type="button" onclick="removeEditFile(${index})" class="ml-3 text-red-600 hover:text-red-700">
    ...
    function removeEditFile(index) {
        ...
        const input = document.getElementById('file_attachments_edit');
